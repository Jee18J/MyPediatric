...
def main():
        ...
        with st.expander("📝 Step 3: Symptom Details", expanded=False):
            ...
            for i, symptom in enumerate(primary_symptoms):
                with st.container():
                    ...
                    if symptom in SYMPTOM_DEFINITIONS and "follow_up" in SYMPTOM_DEFINITIONS[symptom]:
                    ...
                    else: # Handle symptoms without defined follow-up questions
                        severity = st.slider(
                        ...
                        # Store just severity for symptoms without follow-ups (for backward compatibility with PDF code)
    ...
    if st.button("🔍 Analyze Symptoms", type="primary", use_container_width=True):
        if not primary_symptoms:
            ...
            # Populate input features with symptom scores, extracting severity if nested
            for symptom, details in symptom_details.items():
                if symptom in input_features:
                    # Extract the severity score - handle both float and dictionary structures
                    if isinstance(details, dict) and 'severity' in details:
                        input_features[symptom] = details['severity']
                    elif isinstance(details, (int, float)):
                         input_features[symptom] = details
                    # Optional: Add a warning if an unexpected type is found
                    # else:
                    #     st.warning(f"Unexpected data type for symptom {symptom}: {type(details)}. Skipping.")
